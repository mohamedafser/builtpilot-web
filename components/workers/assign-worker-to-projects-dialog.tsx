...
export function AssignWorkerToProjectsDialog({
  workerId,
  workerName,
  open,
  onClose,
  ...
  useEffect(() => {
    ...
    void requestJson<{ projects: Project[] }>(
      `/api/workers/${workerId}/available-projects`,
    ).then((result) => {
      if (cancelled) {
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:px-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="assign-projects-title"
      ...
      >
        <div className="border-b border-stone-100 p-5">
          <h2
            id="assign-projects-title"
            className="text-base font-semibold text-stone-900"
          >
            Assign to project
          </h2>
          <p className="mt-1 text-sm text-stone-500">
            Choose jobs to assign {workerName} to. Already assigned projects
            are hidden.
          </p>
        </div>
        <div className="max-h-[50vh] overflow-y-auto p-5">
          {isLoading ? (
            ...
            <p className="text-sm text-stone-500">
              No available projects. Create a project first, or this worker is
