...
export function VerifyResetForm({
  ...
  useEffect(() => {
    ...
    async function loadSession() {
      ...
      if (!result.ok) {
        setFormError(
          result.message || AUTH_MESSAGES.forgotPasswordMissingSession,
        );
  ...
  function onVerify() {
    ...
    startVerifyTransition(async () => {
      ...
      setSuccessMessage(result.message || "Code verified.");
      router.push(result.data.redirectTo || "/reset-password");
      router.refresh();
    });
  }
  ...
  function onResend() {
    ...
    startResendTransition(async () => {
      ...
      setOtpKey((key) => key + 1);
      setSuccessMessage(
        result.message || AUTH_MESSAGES.forgotPasswordResendSuccess,
      );
      setNow(Date.now());
    });
  }
  ...
  if (!email) {
    return (
      <div className="space-y-4">
        <Alert variant="error">
          {formError || AUTH_MESSAGES.forgotPasswordMissingSession}
        </Alert>
        <p className="text-center text-sm text-stone-500">
          <Link
