...
export function AssignMaterialToProjectsDialog({
  materialId,
  materialName,
  open,
  onClose,
  ...
  useEffect(() => {
    ...
    void requestJson<{ projects: Project[] }>(
      `/api/materials/${materialId}/available-projects`,
    ).then((result) => {
      if (cancelled) {
        return;
      }
  ...
  function assign() {
    ...
    startTransition(async () => {
      const result = await requestJson<{ ids: string[] }>(
        `/api/materials/${materialId}/assign`,
        {
          method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:px-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="assign-material-projects-title"
      ...
      >
        <div className="border-b border-stone-100 p-5">
          <h2
            id="assign-material-projects-title"
            className="text-base font-semibold text-stone-900"
          >
            Assign to project
          </h2>
          <p className="mt-1 text-sm text-stone-500">
            Choose jobs to assign {materialName} to. Already assigned projects
            are hidden.
          </p>
        </div>
        <div className="max-h-[50vh] overflow-y-auto p-5">
          {isLoading ? (
            ...
            <p className="text-sm text-stone-500">
              No available projects. Create a project first, or this material is
              already assigned to every active job.
            </p>
          ) : (
